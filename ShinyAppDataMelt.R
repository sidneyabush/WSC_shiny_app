...
# Going to use Nick's workflow for mergining C and Q data from the google drive, but skip the step
...
chemQ$Date<-as.Date(chemQ$Date) #format date as date

# extract the year and convert to numeric format
chemQ$year <- as.numeric(format(chemQ$Date, "%Y"))

chemQ$unique<-paste0(chemQ$Stream_Name, chemQ$year, chemQ$variable) #create a column of "unique" values - type of sample and date collected
chemQ_unique <-chemQ[!duplicated(chemQ$unique),] #remove all duplicated samples
...
chemQ_final <- chemQ_final %>% filter(grepl("DSi|Ca|Mg|NOx|SO4|PO4|TN|TP", variable))
